            ...
            if(dimension.NumberOfSegments >1) 
            {
                using (Transaction trans = new Transaction(document, "Remove Dimension Segment"))
                {
                    ...
                    try
                    {
                        ...
                        dynamic deletedSegmentIndex = DimensionUtils.GetReferenceIndixByPoint(dimension, point);
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            using (Transaction trans = new Transaction(document, "Split Dimension"))
            {
                ...
                try
                {
                    ...
                    dynamic splitSegmentIndex = DimensionUtils.GetReferenceIndixByPoint(dimension, point);
