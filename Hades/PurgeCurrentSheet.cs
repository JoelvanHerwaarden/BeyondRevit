        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            using (Transaction transaction = new Transaction(doc, "Purge Current Sheet"))
            {
                ...
                foreach (ElementId viewid in viewIds)
                {
                    ...
                    if (view != null)
                    {
                        Utils.Show(view.ViewType.ToString());
                        if (view.ViewType != ViewType.Legend & view.ViewType != ViewType.Schedule & view.ViewType != ViewType.ColumnSchedule & view.ViewType != ViewType.PanelSchedule)
                        {
                            doc.Delete(viewid);
                        }
                    }
