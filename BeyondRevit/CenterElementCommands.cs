...
namespace BeyondRevit.CenterElementCommands
{
    ...
    public class CenterBetweenPoints : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                Utils.ShowInfoBalloon("Center - Move Elements Completed!");
    ...
    public class CenterBetweenPointsPerpendicular : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                Utils.ShowInfoBalloon("Center - Align Elements Cancelled");
                return Result.Cancelled;
            }
            Utils.ShowInfoBalloon("Center - Align Elements Completed!");
            return Result.Succeeded;
        }
    }
    [Transaction(TransactionMode.Manual)]
    [Regeneration(RegenerationOption.Manual)]
    public class RedistributeElementsPickLast : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            UIDocument uidoc = commandData.Application.ActiveUIDocument;
            ...
            try
            {
                using (Transaction transaction = new Transaction(doc, "Redistribute Elements Pick Last"))
                {
                    transaction.Start();

                    //Select the Elements to Divide
                    IList<Reference> references = Utils.GetCurrentSelection(uidoc, null, "Select Elements to Redistribute");

                    //Create a Range Between 0 and 1 with the Number of Elements Selected
    ...
    [Transaction(TransactionMode.Manual)]
    [Regeneration(RegenerationOption.Manual)]
    public class RedistributeElementsPickSecond : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            UIDocument uidoc = commandData.Application.ActiveUIDocument;
            ...
            try
            {
                using (Transaction transaction = new Transaction(doc, "Redistribute Elements Pick Second"))
                {
                    transaction.Start();
                    ...
                    List<double> range = CenterElementUtils.CreateRange(0, 1, references.Count);
                    XYZ start = CenterElementUtils.Get2DPointInView(doc.ActiveView, selection.PickPoint("Pick the First Point"));
                    XYZ second = CenterElementUtils.Get2DPointInView(doc.ActiveView, selection.PickPoint("Pick the Second Point"));
                    XYZ newLocation = start;
                    Line line = Line.CreateBound(start, second);
                    XYZ direction = line.Direction.Multiply(line.Length);
                    for (int i = 0; i < references.Count; i++)
                    {
                        Element element = doc.GetElement(references[i]);
                        XYZ elementLocation = CenterElementUtils.Get2DPointInView(doc.ActiveView, CenterElementUtils.GetElementLocation(element));
    ...
    public class RedistributeElementsWithoutEnds : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                using (Transaction transaction = new Transaction(doc, "Redistribute Elements between 2 points"))
                {
                    transaction.Start();
